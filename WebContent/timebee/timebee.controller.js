sap.ui.controller("timebee.timebee", {
	...
	validateContCalendar:function(oEvt){
        			 ...
        			 var userData={
        				 ...
        				 for(var j=0;j<oData.users[i].events['length'];j++){
            				 if(oData.users[i].events[j]){
            					 ...
            					 oData3.push(oData.users[i].events[j].day);
            				 }
     ...
     calendarDate:function(oEvt){
		...
		for(var i=0;i<oData2.events['length'];i++){
				 ...
				 oCalendar.addSpecialDate(oSpecialDate).addStyleClass("sapUiCalItemType03>.sapUiCalItemText");
